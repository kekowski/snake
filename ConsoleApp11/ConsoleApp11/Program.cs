...
namespace Snake
{
    public enum Direction { Stop, gora, dol, lewo, prawo }
    ...
    internal class Program
    {
        ...
        private static void Input()
        {
            ...
            if (key == ConsoleKey.UpArrow) { _direction = Direction.gora; }
            else if (key == ConsoleKey.DownArrow) { _direction = Direction.dol; }
            else if (key == ConsoleKey.LeftArrow) { _direction = Direction.lewo; }
            else if (key == ConsoleKey.RightArrow) { _direction = Direction.prawo; }
        ...
        private static void Logic()
        {
            ...
            if (_direction == Direction.gora)
            {
                if (_head.Y - 1 < 1) { _head.Y = _size.Height - 2; }
                else { _head.Y--; }
            }
            else if (_direction == Direction.dol)
            {
                if (_head.Y + 1 > _size.Height - 2) { _head.Y = 1; }
                else { _head.Y++; }
            }
            else if (_direction == Direction.lewo)
            {
                if (_head.X - 1 < 1) { _head.X = _size.Width - 2; }
                else { _head.X--; }
            }
            else if (_direction == Direction.prawo)
            {
        ...
        {
            $"GAME OVER".Write(_size.Width + 3, 3, ConsoleColor.Red, ConsoleColor.Black);
